...
angular.module('shopnxApp')
  .controller('ProductTNGCtrl', function ($scope, socket, ProductTNG, Category, Brand, Feature, Modal, toastr, $loading, Settings) {
    var cols = [
    ...
    $scope.products = ProductTNG.query({}, function() {
    ...
    $scope.edit = function(product) {
      ...
      Modal.show(product,{title:title, api:'ProductTNG', columns: cols});
    };
    ...
    $scope.save = function(product){
      ...
      if('_id' in product){
          ProductTNG.update({ id:$scope.product._id }, $scope.product).$promise.then(function() {
          ...
          ProductTNG.save($scope.product).$promise.then(function() {
            toastr.success("Product info saved successfully","Success");
          }, function(error) { // error handler
              var err = error.data.errors;
              toastr.error(err[Object.keys(err)].message,err[Object.keys(err)].name);
    ...
    $scope.changeActive = function(b){ // success handler
      b.active = !b.active;
      ProductTNG.update({ id:b._id }, b).$promise.then(function() {
