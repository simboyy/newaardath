...
angular.module('shopnxApp')
  .controller('ProductMPCtrl', function ($scope, socket, ProductMP, CategoryMP, Brand, Feature, Modal, toastr, $loading, Settings) {
    var cols = [
    ...
    $scope.products = ProductMP.query({}, function() {
      $loading.finish('products');
      socket.syncUpdates('product', $scope.products);
    });

    $scope.categories = CategoryMP.query(function() {
    ...
    $scope.edit = function(product) {
      ...
      Modal.show(product,{title:title, api:'ProductMP', columns: cols});
    };

    $scope.delete = function(product) {
      
      if(confirm('Are you sure to delete the product?')){
        ProductMP.delete({id:product._id});
      }
    };
    $scope.save = function(product){
      ...
      if('_id' in product){
          ProductMP.update({ id:$scope.product._id }, $scope.product).$promise.then(function() {
          ...
          ProductMP.save($scope.product).$promise.then(function() {
            toastr.success("Product info saved successfully","Success");
          }, function(error) { // error handler
              var err = error.data.errors;
              toastr.error(err[Object.keys(err)].message,err[Object.keys(err)].name);
    ...
    $scope.changeActive = function(b){ // success handler
      b.active = !b.active;
      ProductMP.update({ id:b._id }, b).$promise.then(function() {
