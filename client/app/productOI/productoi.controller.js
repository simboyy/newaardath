...
angular.module('shopnxApp')
  .controller('ProductOICtrl', function ($scope, socket, ProductOI, Category, Brand, Feature, Modal, toastr, $loading, Settings) {
    var cols = [
    ...
    $scope.products = ProductOI.query({}, function() {
    ...
    $scope.edit = function(product) {
      ...
      Modal.show(product,{title:title, api:'ProductOI', columns: cols});
    };
    ...
    $scope.save = function(product){
      ...
      if('_id' in product){
          ProductOI.update({ id:$scope.product._id }, $scope.product).$promise.then(function() {
          ...
          ProductOI.save($scope.product).$promise.then(function() {
            toastr.success("Product info saved successfully","Success");
          }, function(error) { // error handler
              var err = error.data.errors;
              toastr.error(err[Object.keys(err)].message,err[Object.keys(err)].name);
    ...
    $scope.changeActive = function(b){ // success handler
      b.active = !b.active;
      ProductOI.update({ id:b._id }, b).$promise.then(function() {
