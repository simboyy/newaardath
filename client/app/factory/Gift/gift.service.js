'use strict';
  ...
  function GiftCart(GiftName) {
      this.cartName = GiftName;
    ...
    // items in the cart
    //
    function GiftItem(sku, name, slug, mrp, price, quantity, image, category, size, weight) {
      // console.log(size);
        this.sku = sku;
        this.name = name;
        this.slug = slug;
  ...
  GiftCart.prototype.loadItems = function () {
      var items = localStorage !== null ? localStorage[this.GiftName + '_items'] : null;
          ...
          try {
              ...
              for (var i = 0; i < items.length; i++) {
                  ...
                  if (item.sku !== null && item.name !== null && item.price !== null) {
                      item = new GiftItem(item.sku, item.name, item.slug, item.mrp, item.price, item.quantity, item.image, item.category, item.size, item.weight, item.status);
  ...
  GiftCart.prototype.saveItems = function () {
      if (localStorage !== null && JSON !== null) {
          localStorage[this.GiftName + '_items'] = JSON.stringify(this.items);
      }
  };

  // adds an item to the cart
  GiftCart.prototype.addItem = function (product,quantity) {
    ...
    // sku, name, slug, mrp, price, quantity, image, category, size, weight
      ...
      if (quantity !== 0) {
          ...
          // new item, add now
          if (!found) {
              var itm = new GiftItem(product.sku, product.name, product.slug, product.mrp, product.price, product.quantity, product.image, product.category, product.size, product.weight, 0);
              this.items.push(itm);
  ...
  GiftCart.prototype.getTotalWeight = function (sku) {
      var totalWeight = 0;
  ...
  // get the total price for all items currently in the cart
  GiftCart.prototype.getTotalPrice = function (sku) {
      var total = 0;
      for (var i = 0; i < this.items.length; i++) {
          var item = this.items[i];
          if (sku === undefined || item.sku === sku) {
  ...
  // get the total price for all items currently in the cart
  GiftCart.prototype.getShippingCharge = function (shipping) {
  ...
  GiftCart.prototype.getTotalPriceAfterShipping = function (shipping,couponAmount) { //Total Price Including Shipping
  ...
  // get the total price for all items currently in the cart
  GiftCart.prototype.getTotalCount = function (sku) {
      var count = 0;
      for (var i = 0; i < this.items.length; i++) {
          var item = this.items[i];
          if (sku === undefined || item.sku === sku) {
  ...
  // clear the cart
  GiftCart.prototype.clearItems = function () {
      this.items = [];
      this.skuArray = [];
      this.saveItems();
  };

  GiftCart.prototype.toNumber = function (value) {
      value = value * 1;
      return isNaN(value) ? 0 : value;
  };

  // define checkout parameters
GiftCart.prototype.addCheckoutParameters = function (serviceName, merchantID, options) {
...
// check out
GiftCart.prototype.checkout = function (serviceName, clearCart) {
...
GiftCart.prototype.checkoutPayPal = function (parms, clearCart) {
...
GiftCart.prototype.checkoutGoogle = function (parms, clearCart) {
...
// check out using COD
GiftCart.prototype.checkoutCOD = function (parms, clearCart) {
...
GiftCart.prototype.checkoutStripe = function (parms, clearCart) {
  ...
  // utility methods
  GiftCart.prototype.addFormFields = function (form, data) {
      if (data !== null) {
  ...
  angular.module('shopnxApp')
    .factory('Gift', function (Setting, $location) {
      var myGift = new GiftCart('Gift');
