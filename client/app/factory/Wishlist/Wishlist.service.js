'use strict';
  ...
  function WishlistCart(wishlistName) {
      this.wishName = wishlistName;
    ...
    // items in the cart
    //
    function WishlistItem(sku, name, slug, mrp, price, quantity, image, category, size, weight) {
      // console.log(size);
        this.sku = sku;
        this.name = name;
        this.slug = slug;
  ...
  WishlistCart.prototype.loadItems = function () {
      var items = localStorage !== null ? localStorage[this.wishlistName + '_items'] : null;
          ...
          try {
              ...
              for (var i = 0; i < items.length; i++) {
                  ...
                  if (item.sku !== null && item.name !== null && item.price !== null) {
                      item = new WishlistItem(item.sku, item.name, item.slug, item.mrp, item.price, item.quantity, item.image, item.category, item.size, item.weight, item.status);
  ...
  WishlistCart.prototype.saveItems = function () {
      if (localStorage !== null && JSON !== null) {
          localStorage[this.wishlistName + '_items'] = JSON.stringify(this.items);
      }
  };

  // adds an item to the cart
  WishlistCart.prototype.addItem = function (product,quantity) {
    ...
    // sku, name, slug, mrp, price, quantity, image, category, size, weight
      ...
      if (quantity !== 0) {
          ...
          // new item, add now
          if (!found) {
              var itm = new WishlistItem(product.sku, product.name, product.slug, product.mrp, product.price, product.quantity, product.image, product.category, product.size, product.weight, 0);
              this.items.push(itm);
  ...
  WishlistCart.prototype.getTotalWeight = function (sku) {
      var totalWeight = 0;
  ...
  // get the total price for all items currently in the cart
  WishlistCart.prototype.getTotalPrice = function (sku) {
      var total = 0;
      for (var i = 0; i < this.items.length; i++) {
          var item = this.items[i];
          if (sku === undefined || item.sku === sku) {
  ...
  // get the total price for all items currently in the cart
  WishlistCart.prototype.getShippingCharge = function (shipping) {
  ...
  WishlistCart.prototype.getTotalPriceAfterShipping = function (shipping,couponAmount) { //Total Price Including Shipping
  ...
  // get the total price for all items currently in the cart
  WishlistCart.prototype.getTotalCount = function (sku) {
      var count = 0;
      for (var i = 0; i < this.items.length; i++) {
          var item = this.items[i];
          if (sku === undefined || item.sku === sku) {
  ...
  // clear the cart
  WishlistCart.prototype.clearItems = function () {
      this.items = [];
      this.skuArray = [];
      this.saveItems();
  };

  WishlistCart.prototype.toNumber = function (value) {
      value = value * 1;
      return isNaN(value) ? 0 : value;
  };

  // define checkout parameters
WishlistCart.prototype.addCheckoutParameters = function (serviceName, merchantID, options) {
...
// check out
WishlistCart.prototype.checkout = function (serviceName, clearCart) {
...
WishlistCart.prototype.checkoutPayPal = function (parms, clearCart) {
...
WishlistCart.prototype.checkoutGoogle = function (parms, clearCart) {
...
// check out using COD
WishlistCart.prototype.checkoutCOD = function (parms, clearCart) {
...
WishlistCart.prototype.checkoutStripe = function (parms, clearCart) {
  ...
  // utility methods
  WishlistCart.prototype.addFormFields = function (form, data) {
      if (data !== null) {
  ...
  angular.module('shopnxApp')
    .factory('Wishlist', function (Setting, $location) {
      var myWishlist = new WishlistCart('Wishlist');
