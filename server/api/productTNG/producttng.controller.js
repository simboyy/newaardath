...
exports.count = function(req, res) {
  if(req.query){
    ...
    ProductTNG.find(q).count().exec(function (err, count) {
...
exports.index = function(req, res) {
  if(req.query){
    ...
    // setTimeout(function(){
      ProductTNG.find(q).limit(req.query.limit).skip(req.query.skip).sort(sort).select(select).exec(function (err, products) {
        if(err) { return handleError(res, err); }
        return res.status(200).json(products);
      });
    // },2000);
  }else{
    ProductTNG.find(function (err, products) {
      if(err) { return handleError(res, err); }
      return res.status(200).json(products);
    });
  }
...
exports.show = function(req, res) {
  ProductTNG.findById(req.params.id, function (err, product) {
    if(err) { return handleError(res, err); }
...
exports.create = function(req, res) {
  ...
  if(!req.body.slug && req.body.name)
    req.body.slug = req.body.name.toString().toLowerCase()
                      ...
                      .replace(/-+$/, '');
  ProductTNG.create(req.body, function(err, product) {
    if(err) { return handleError(res, err); }
